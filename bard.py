...
def generate_itinerary(source, destination, start_date, end_date, no_of_day, budget):
    ...
    prompt = f"Generate a personalized trip itinerary for a {no_of_day}-day trip {source} to {destination} from {start_date} to {end_date}, with an exact budget {budget} INR."
